...
export function CitizenDashboard({
  ...
  const [newFamilyMember, setNewFamilyMember] = useState({ name: '', relation: 'Family Member', phone: '' });
  ...
  const handleTriggerSOS = () => {
    ...
    setTimeout(() => {
      alert(`🚨 EMERGENCY SOS DISPATCHED!\n\nYour GPS Location (${userLocation?.lat.toFixed(4)}, ${userLocation?.lng.toFixed(4)}) and emergency distress call have been broadcast to:\n- Kerala Police ERSS (112)\n- Wayanad District Disaster Control Room (1077)\n- NDRF Local Rapid Response Unit`);
  ...
  const handleAddFamilySubmit = (e: React.FormEvent) => {
    ...
    setNewFamilyMember({ name: '', relation: 'Family Member', phone: '' });
  };
      ...
      case 'family':
        return (
          <div className="space-y-6 animate-fade-in max-w-4xl">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-black text-slate-900">Family Safety Status</h1>
                <p className="text-xs text-slate-500">Monitor live check-in status of your registered family members</p>
              </div>
              <button
                onClick={() => setAddFamilyModalOpen(true)}
                className="text-xs font-bold text-[#0E8F66] hover:bg-emerald-50 px-3.5 py-2 rounded-xl border border-emerald-200 transition-colors"
              >
                + Add Member
              </button>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-6 space-y-5">
              <div className="bg-[#EAF8F3] border border-emerald-200/80 p-4 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#0E8F66] text-white flex items-center justify-center">
                    <Heart className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-[#0B4D3B]">Your Current Status</p>
                    <p className="text-xs text-emerald-800">
                      {userIsSafe ? `Marked SAFE &bull; ${lastSafeTime}` : 'Distress Alert Active'}
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                {familyMembers.length === 0 ? (
                  <div className="p-10 text-center space-y-3">
                    ...
                    <p className="text-xs text-slate-500 max-w-md mx-auto">
                      Click "+ Add Member" above to register family safety check-ins and shelter status.
                    </p>
                  </div>
                ) : (
                  familyMembers.map((member) => (
                    <div key={member.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-200/60 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{member.relation} &bull; {member.location}</p>
                        </div>
                      </div>

                      <div className="text-right">
                        <span
                          className={`text-xs font-bold px-3 py-1 rounded-full ${member.status === 'Safe'
                              ? 'bg-emerald-100 text-[#0B4D3B]'
                        ...
                        <p className="text-[11px] text-slate-400 mt-1">{member.lastCheckin}</p>
                      </div>
                    </div>
                  ))
                )}
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans selection:bg-emerald-100 selection:text-emerald-900">
      ...
      {addFamilyModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl space-y-4 animate-slide-down">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-base text-slate-900">Add Family Member</h3>
              <button onClick={() => setAddFamilyModalOpen(false)} className="p-1 text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddFamilySubmit} className="space-y-3 text-xs">
              ...
              <div className="pt-2 flex items-center gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-[#0E8F66] text-white font-bold rounded-xl shadow-xs"
                >
                  Save Member
                </button>
              </div>
